...
// ── Servo support ──────────────────────────────────────────────────────────
// myServo is defined in the .ino file; we just reference it here.
...
// ══════════════════════════════════════════════════════════════════════════════
//  JPEG chunk encoder callback
// ══════════════════════════════════════════════════════════════════════════════
static size_t jpg_encode_stream(void *arg, size_t index,
...
// ══════════════════════════════════════════════════════════════════════════════
//  Camera control command handler  (/control?var=xxx&val=yyy)
// ══════════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════════
//  Register write handler  (/reg?reg=<r>&mask=<m>&val=<v>)
// ══════════════════════════════════════════════════════════════════════════════
static esp_err_t reg_handler(httpd_req_t *req) {
  ...
  log_i("Set Register: reg: 0x%02x, mask: 0x%02x, value: 0x%02x",
        reg, mask, val);
  sensor_t *s   = esp_camera_sensor_get();
...
// ══════════════════════════════════════════════════════════════════════════════
//  Register read handler  (/greg?reg=<r>&mask=<m>)
// ══════════════════════════════════════════════════════════════════════════════
static esp_err_t greg_handler(httpd_req_t *req) {
  ...
  if (res < 0) return httpd_resp_send_500(req);
  log_i("Get Register: reg: 0x%02x, mask: 0x%02x, value: 0x%02x",
        reg, mask, res);
...
// ══════════════════════════════════════════════════════════════════════════════
//  PLL handler  (/pll?bypass=&mul=&sys=&root=&pre=&seld5=&pclken=&pclk=)
// ══════════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════════
//  Window / resolution handler  (/resolution?sx=&sy=&ex=&ey=&...)
// ══════════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════════
//  ★  SERVO HANDLER  (/servo?angle=<0-180>)                                ★
//
//  Called by the Python ServoTracker class.
//  myServo is declared in the .ino and extern'd at the top of this file.
//
//  Example:  GET http://192.168.43.8/servo?angle=90
// ══════════════════════════════════════════════════════════════════════════════
static esp_err_t servo_handler(httpd_req_t *req) {
  ...
  // Safety clamp — never send the servo out of range
  if (angle < 0)   angle = 0;
...
// Helper macro: fill an httpd_uri_t in one shot
#ifdef CONFIG_HTTPD_WS_SUPPORT
#define MAKE_URI(name_, uri_, method_, handler_)           \
  httpd_uri_t name_ = {                                    \
    .uri                     = uri_,                       \
    .method                  = method_,                    \
    .handler                 = handler_,                   \
...
void startCameraServer() {
  ...
  config.max_uri_handlers  = 16;  // enough for all handlers including /servo

  // ── URI definitions ──────────────────────────────────────────────────────
  MAKE_URI(index_uri,   "/",           HTTP_GET, index_handler);
  MAKE_URI(status_uri,  "/status",     HTTP_GET, status_handler);
  MAKE_URI(cmd_uri,     "/control",    HTTP_GET, cmd_handler);
  MAKE_URI(capture_uri, "/capture",    HTTP_GET, capture_handler);
  MAKE_URI(stream_uri,  "/stream",     HTTP_GET, stream_handler);
  MAKE_URI(bmp_uri,     "/bmp",        HTTP_GET, bmp_handler);
  MAKE_URI(xclk_uri,    "/xclk",       HTTP_GET, xclk_handler);
  MAKE_URI(reg_uri,     "/reg",        HTTP_GET, reg_handler);
  MAKE_URI(greg_uri,    "/greg",       HTTP_GET, greg_handler);
  MAKE_URI(pll_uri,     "/pll",        HTTP_GET, pll_handler);
  MAKE_URI(win_uri,     "/resolution", HTTP_GET, win_handler);
  MAKE_URI(servo_uri,   "/servo",      HTTP_GET, servo_handler); // ← NEW

  ra_filter_init(&ra_filter, 20);

  // ── Camera HTTP server (port 80) ─────────────────────────────────────────
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &servo_uri);  // ← NEW
  }
